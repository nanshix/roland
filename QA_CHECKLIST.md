...
2. Verify navigation: `Home` (brand) -> `Footy Timer` -> `More Helpers` card works and returns correctly.
...
11. Switch to `Home` while timer is running; return to `Footy Timer` and confirm no runaway clock/duplicate intervals.
