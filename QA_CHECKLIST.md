...
2. Verify nav switching: `Timer` <-> `More Tools` works and returns correctly.
...
11. Switch to `More Tools` while timer is running; return to `Timer` and confirm no runaway clock/duplicate intervals.
